...
def get_fmi_data(fmisid, info):
    ...
    start = now - timedelta(hours=6) # 6h for in case of missing values
...
# Get old data (in case of missing values)
...
def preprocess_batch(df, db):
    ...
    for sid, group in df.groupby('station_id'):
        # If FMI is missing data, try to fill from MongoDB
        if len(group) < 4:
        ...
        # If we STILL don't have enough, use ffill
    ...
    try:
        ...
        for _, row in clean_df.iterrows():
            ...
            doc = {
                ...
                "actual_data": {f: row_dict[f] for f in features},
                "predictions": {
